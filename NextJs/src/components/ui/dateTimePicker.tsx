...
import { generateAIDateAction } from "@/features/dateTime/generateAiDate.action";
        ...
        CaptionLabel: ({ displayMonth }) => {
          return (
            <div className="inline-flex w-full gap-4">
              ...
              >
                <SelectTrigger variant="accent" size="sm" className="w-fit">
                  <SelectValue />
                </SelectTrigger>
...
const DateTimePicker = React.forwardRef<DateTimePickerRef, DateTimePickerProps>(
  (
    ...
    const getDateMutation = useMutation({
      mutationFn: async (prompt: string) => {
        const localDateUTC = new Date().toUTCString();
        const result = await generateAIDateAction({
          localDateUTC,
          prompt,
          timeZoneStr,
        });
          ...
          <InputUnit
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.stopPropagation();
                e.preventDefault();
                getDateMutation.mutate(inputValue);
              }
            }}
          />
